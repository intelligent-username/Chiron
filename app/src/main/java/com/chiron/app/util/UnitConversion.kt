...
object UnitConversion {
    ...
    fun formatSet(
        ...
        val weightPart = if (hasWeight && set.weightLbs != null) {
            if (displayInKg) "${formatNumber(lbsToDisplayKg(set.weightLbs))}kg"
            else "${formatNumber(set.weightLbs)}lbs"
        } else if (hasWeight) null else null

        val repOrTimePart = when {
            hasReps && set.reps != null -> "${set.reps}"
            hasTime && set.durationSeconds != null -> formatDuration(set.durationSeconds)
            else -> null
        }

        val distPart = if (hasDist && set.distanceMeters != null) {
            formatDistance(set.distanceMeters, distanceUnit)
        } else null
