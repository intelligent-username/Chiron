...
fun EditSetDialog(
    ...
    var weightField by remember {
        mutableStateOf(
            if (hasWeight && weightLbs != null) {
                val v = if (displayInKg) UnitConversion.lbsToDisplayKg(weightLbs) else weightLbs
    ...
    var distanceField by remember {
        mutableStateOf(
            if (hasDist && distanceMeters != null) {
                val v = if (distanceUnit == DistanceUnit.FEET)
                    UnitConversion.metersToFeet(distanceMeters) else distanceMeters
                UnitConversion.formatNumber(v)
            } else ""
        )
    }
